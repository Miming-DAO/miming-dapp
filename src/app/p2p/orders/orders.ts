...
export class Orders {
  ...
  get filteredOrders(): P2pOrder[] {
    ...
    if (this.activeStatusTab === 'in-progress') {
      filtered = filtered.filter(order => order.status === 'pending' || order.status === 'paid' || order.status === 'notified');
    } else {
  ...
  getStatusBadgeClass(status: string): string {
    const classes: any = {
      ...
      'notified': 'bg-orange-500/20 text-orange-400 border-orange-500/30',
