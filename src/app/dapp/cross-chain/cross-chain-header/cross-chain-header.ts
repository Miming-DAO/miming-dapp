...
@Component({
  selector: 'app-cross-chain-header',
  imports: [
    CommonModule,
    PToastModule,
    PDialogModule,
  ...
  templateUrl: './cross-chain-header.html',
  styleUrl: './cross-chain-header.css',
  providers: [MessageService],
})
export class CrossChainHeader {
  ...
  async connectXteriumWallet() {
    try {
      if (this.isMobileDevice) {
        const callbackUrl = window.location.origin + '/dapp/cross-chain-xterium-accounts';
  ...
  logoutPolkadotWalletAccount(): void {
    ...
    setTimeout(() => {
      ...
      this.router.navigate(['/dapp/cross-chain']).then(() => {
        location.reload();
      });
    }, 500);
  }
